...
from .mesh import network, device
import logging
import re
...
class acync:
    ...
    def _get_properties(auth_token, product_id, device_id):
        ...
        headers = {'Access-Token': auth_token}
        r = requests.get(API_DEVICE_INFO.format(product_id=product_id, device_id=device_id), headers=headers,
                         timeout=acync.API_TIMEOUT)
        return r.json()
    ...
    def app_meshinfo_to_configdict(meshinfo):
        ...
        for mesh in meshinfo:
            ...
            for bulb in mesh['properties']['bulbsArray']:
                if any(
                    checkattr not in bulb for checkattr in ('deviceID', 'displayName', 'mac', 'deviceType')): continue
    ...
    def __init__(self, **kwargs):
        ...
        self.callback: Cync2MQTT.callback_routine = kwargs.get('callback')
    ...
    def populate_from_jsonfile(self, jsonfile):
        ...
        for mesh in self.xlinkdata:
            ...
            for bulb in mesh['properties']['bulbsArray']:
                ...
                self.devices[f"{mesh['mac']}/{id}"] = device(mesh_network, bulb['displayName'], id, bulb['mac'],
                                                             bulb['deviceType'])
